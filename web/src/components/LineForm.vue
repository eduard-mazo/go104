...
watch(() => props.open, (v) => {
  if (v) {
    form.value = props.line
      ? { ...props.line }
      : { port: 2404, common_address: 1, k: 12, w: 8, t1_ms: 15000, t2_ms: 10000, t3_ms: 20000, gi_interval_s: 900, enabled: true }
    error.value = ''
  }
})
        ...
        <div class="relative w-full max-w-lg card p-6 shadow-2xl space-y-5">
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input v-model.number="form.port" type="number" class="input-base" placeholder="2404" />
            </div>
            ...
            <div>
              <label class="block text-xs font-medium text-slate-400 mb-1">GI Interval (s, 0=off)</label>
              <input v-model.number="form.gi_interval_s" type="number" class="input-base" />
            </div>

            <fieldset class="col-span-2 border border-slate-600 rounded-md p-3 space-y-3">
